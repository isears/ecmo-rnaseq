...
if __name__ == "__main__":
    ...
    data_path = "/gpfs/home/isears1/Repos/ecmo-rnaseq/outputs/dartmouthpipeline/"
    merged_counts = pd.DataFrame()
    count_cols = list()

    for fname in glob.glob("outputs/dartmouthpipeline/count_results/*_counts.txt"):
        sample_id = fname.split("/")[-1].split("_")[0]
        print(f"Analysing {sample_id}")
        counts_df = pd.read_csv(fname, sep="\t", skiprows=1)
